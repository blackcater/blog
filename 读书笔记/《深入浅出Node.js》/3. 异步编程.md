...
- **异步调用的依赖处理**
  ...
  `series()`适合无依赖的异步串行执行，但当前一个结果是后一个调用的输入时，`series()`方法就无法满足需求了。
  ```javascript
  async.waterfall([
    function(callback) {
      fs.readFile('file1.txt', 'utf8', callback);
...
- **自动依赖处理**
  ...
  现实业务中，具有很多复杂的依赖关系，这些业务或是异步或是同步。编程环境中可能会让人处于理不清顺序的情况。
  ...
  `auto()`方法根据依赖关系自动分析，以最佳顺序执行上面业务：

  ```javascript
  async.auto(deps);
  ```

  转换到EventProxy的实现，则需要更细腻的事件分配：
  ...
  }).all('connectMongoDB', 'connectRedix', 'uploadAsserts', function(){
    // Start up
  });
  ```
...
另一个知名的流程控制库是Time Caswell的`Step`，它比async更加的轻量。

Step接受任意数量的任务，所有的人物都将会串行一次执行。
...
可以看到Step与前面介绍的事件模式，Promise甚至async不同一点在于Step用到了this关键字。事实上，它是Step内部的一个`next()`方法，将异步调用的结果传递给下一个任务作为参数，并调用执行。
...
- **并行任务执行**
  ...
  使用`parallel()`的时候需要小心的是，如果异步方法的结果传回的是多个参数，Step将只会取前两个参数。
  ...
  Step的`parallel()`原理是每次执行时将内部的计数器加1，然后返回一个回调函数，这个回调函数在异步调用结束时才执行。当回调函数执行时，将计数器减1。当计数器为0时，告知step所有异步调用结束，Step会执行下一个方法。

- **结果分组**

  Step提供另一个方法叫`group()`， 它类似于`parallel()`的效果，但是在结果传递上略有不同。下面为读取一个目录：
  ...
  有两次`group()`调用、第一次告知Step要执行并行执行，第二次调用的结果将会生成一个回调函数，而回调函数接受的返回值将会按组存储。`parallel()`传递的形式如下:
...
这里还要介绍一种思路完全不相同的异步编程方案`wind` 。其前身为`Jscex`，由国内知名码农赵某开发完成。其为HS语言提供了一个`monadic`扩展，能够显著提升一些场景下的异步编程体验。
...
现在我们要添加需求就是让冒泡排序动画起来。这意味着swap()内要添加动画逻辑。这并不困难，但困难的是动画需要延迟，我们知道JS中只有setTimeout类函数可以实现延迟(while不行，前面提过)，但他们是异步方法，在执行后立即返回，所以难点如下：

- 动画执行时无法停止排序算法执行
- 排序算法的继续执行将会启动更多的动画

因此，逐步骤的动画将难以实现，而wind在解决这种问题上体现它独特魅力：
...
- **异步任务定义**
  ...
  `eval`在函数业界被定义为魔鬼，因为它可以导致上下文的混乱。

  但在`wind`的世界里，恰好相反。巧妙的应用`eval()`访问上下文的特性。`Wind.comile()`会将普通的函数进行编译，然后交给`eval()`执行。换言之`eval(Wind.compile('async', function(){}))`定义了异步任务。`Wind.Async.sleep()`则内置了对setTimeout的封装。

- **$await与任务模型**

  定义完异步方法后，wind提供了`$await()`方法实现等待完成异步方法。事实上它并不是一个方法，也不存在于上下文中，只是一个等待的占位符，告知表明一起这里需要等待。

  `$await`接受的参数为一个任务对象，表示等待任务结束后才会执行后续的操作。每一个异步操作都能转化为一个任务，`wind`正是基于任务模型实现的。
  ...
  除了通过`eval(Wind.compile('async', function(){}))`定义任务外，正式的任务创建方法为`Task.create()`。执行`readFileAsync`进行偏函数转化得到真正的任务。异步方法在执行结束时，可以通过`complete()`传递`failure, success`信息，告知任务执行完毕。如果`failure`则通过`try/catch`捕获异常。

  下面为调用`readFileAsync`得到一个任务的实例:
  ...
  var serial = eval(Wind.compile('async', function(){
    var data1 = $await(readFileAsync('file1.txt', 'utf8'));
    console.log(data1);
    var data2 = $await(readFileAsync('file2.txt', 'utf8'));
    console.log(data2);
    var data3 = $await(readFileAsync('file3.txt', 'utf8'));
    console.log(data3);
  }));
  ...
  异步方法在JS中通常会立即返回，在wind中做到了不阻塞CPU但阻塞代码的目的。接下来我们尝试一下并行效果:
...
- **异步方法转化的辅助函数**
  ...
  可以看到，除了`eval(Wind.compile('async', function(){}))`在实际代码中稍显冗长外，异步调用在代码层面上已经与同步调用相差无几。

  如果Promise/Deferred模型可以让异步编程模型变简单，这种近同步编程的体验需要我们额外或者提前完成的事情是：将异步方法任务化。这种任务化的过程可以看做是Promise/Deferred模型的封装。如果每个方法都如readFileAsync一样去定义，将会是一个庞大的工程。wind提供两个方法来辅助转化：

  - Wind.Async.Binding.fromCallback
  - Wind.Async.Binding.fromStandard

  在Node中异步方法的回调传值有两种，一种是无异常回调，通常只有一个参数返回，而fromCallback用于转化这类异步调用为wind中的任务。

  另一类是带异常调用，遵循规范将返回参数列表的第一个参数作为异常标示，而fromStandard用于转化这类异步调用到wind中的任务。
...
这里简单对比一下几种方案的区别：事件发布/订阅模式相对算是一种较为原始的方式，Promise/Deferred模式贡献了一个非常不错的异步任务模型的抽象。而上述这些异步流程控制方案与Promise/Deferred模式的思路不同，Promise/Deferred的重头在于封装异步的调用部分，流程控制库则显得没有模式，将处理重点放置在回调函数的注入上。从自由度上来进。`async, Step`这类流程库要相对灵活得多。EventProxy库则主要借鉴事件发布/订阅模式和流程控制库通过高阶函数生成回调函数方式实现。
...
如果想对既有的异步API添加过载保护，我们期望的当然不是修改API。bagpipe模块给出的解决思路如下：
...
bagpipe的API主要暴露了一个push()方法和full事件，实例代码如下：

```javascript
var Bagpipe = require('bagpipe');
// 设定最大并发数
var bagpipe = new Bigpipe(10);
for(var i=0; i<100; i++){
...
push()依然是通过函数变换的方式实现的，假设第一个参数为方法，最后一个参数为回调函数，其余为其他参数，其核心实现如下：
...
将调用推入队列后，调用一次next()方法尝试触发。next()定义如下:

```javascript
Bagpipe.prototype.next = function(){
  var that = this;
  if(this.active < that.limit && that.queue.length) {
...
next()方法主要判断活跃调用的数量，如果正常，将调用内部方法run()来执行真正的调用。这里为了判断回调函数是否执行，采用了一个注入代码的技巧：
...
用户传入回调函数被真正执行前，被封装替换过。这个封装的回调函数内部的逻辑将活跃值的计数器减1后，主动调用next()执行后续等待的异步调用。

- **拒绝模式**

  bagpipe还有一些深度的使用方式。对于大量的异步调用，也需要分场景进行区分，因为涉及并发控制，必然会造成部分调用需要进行等待。如果调用有实时方面的要求，那么需要快速返回，因为等待的方法被真正执行时，可能已经超过了等待时间，即使返回数据也没有用了。这种场景下需要快速失败，让调用方尽早返回，而不用浪费不必要的等待时间。bagpipe支持拒绝模式。
  ...
  在拒绝模式下，如果等待的调用队列也满了之后，新来的调用就直接返回它一个队列太忙的拒绝异常。

- **超时控制**

  造成队列拥塞的主要原因是一步调用耗时太久，调用产生的速度远远高于执行的速度。为了防止某个异步调用使用太多的时间，我们需要设置一个时间基线。将那些执行太久的异步调用清理出活跃队列。否则在拒绝模式下，会有太多的调用因为某个执行得慢，导致得到拒绝异常。
...
`parallelLimit()`方法的缺陷在于无法动态的添加并行任务。为此，async提供了queue方法，来满足该需求。
...
尽管queue()实现动态添加并行任务，但是相比parallelLimit()，由于queue()接受参数固定，其丢失了parallelLimit()的多样性。