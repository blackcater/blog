...
````javascript
let x1 = new Int8Array([1, 2, 3, 4, 5, 6, 7, 8]);
let x2 = new Int32Array(x1);
console.log(x2.buffer.byteLength); // 32 => 4*8

// Int32所以一个数字为4个字节，结果是32说明有8个，而传入的x1如果以原空间进行拷贝，则只有2个但是有8个，和创建x1使用的数组长度相同，所以就解释通了。
````
...
// 上面代码中，数组y是以数组x为模板而生成的，当x变动的时候，y并没有变动。所以是新开的内存存储数据
```
